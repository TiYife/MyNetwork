...
def train():
    ...
    data_train = MNIST('data/MNIST_DATA/', train=True, download=True, transform=transform)
    data_valid = MNIST('data/MNIST_DATA/', train=False, download=True, transform=transform)
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            if valid_loss < best_loss:
                ...
                torch.save(model.state_dict(), 'output/best_model_mnist')
                print(f"Model {best_epoch} saved")
...
def run():
    state = torch.load('output/best_model_mnist')
    model = VAE()
